...
- Disabled Codex Responses Lite metadata for OpenAI API catalog entries and normalized residual `reasoning.context = "all_turns"` requests to `auto`, preventing older `o3` models from rejecting the request while preserving provider-selected reasoning context.
